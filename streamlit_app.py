# streamlit_app.py
# Mega Millions patterns dashboard (Streamlit) with PDF & HTML export
...
WHITE_MAX = 70
MEGA_MAX = 25
NY_OD_URL = "https://data.ny.gov/api/views/5xaw-6ayf/rows.csv?accessType=DOWNLOAD"

st.set_page_config(page_title="Mega Millions Patterns", layout="wide")
...
def fetch_history_years(years: int) -> pd.DataFrame:
    ...
    raw["Draw Date"] = pd.to_datetime(raw["Draw Date"])
    raw = raw.sort_values("Draw Date")
    cutoff = datetime.today() - timedelta(days=int(365.25 * years))
    raw = raw[raw["Draw Date"] >= cutoff].copy()
    whites = (
        raw["Winning Numbers"]
        .astype(str)
...
show_cooccur = st.sidebar.checkbox(
    "Show pair co-occurrence heatmap (70×70)", value=False
)
...
st.title("Mega Millions • Patterns & Heat Maps")
st.caption(
...
with tab1:
    ...
    st.subheader("Mega Ball (Recency-Weighted)")
    df_mega = pd.DataFrame(mega_rank_recent[:10], columns=["mega", "recency_prob"])
    ...
    with coldl[2]:
        st.download_button(
            "⬇️ Mega ranks (recency)",
            data=df_to_csv_bytes(
                pd.DataFrame(mega_rank_recent, columns=["mega", "recency_prob"])
            ),
            file_name="rank_mega_recency.csv",
        )
    with coldl[3]:
        st.download_button(
...
with tab2:
    ...
    grid_p = gridify_1d(w_plain, 10, 7)
    ...
    grid_r = gridify_1d(w_recent, 10, 7)
    ...
    st.subheader("Mega Ball (Recency-Weighted) — Bar")
    fig3, ax3 = plt.subplots()
    xs = np.arange(1, MEGA_MAX + 1)
    ax3.bar(xs, m_recent[1:])
    ax3.set_xlabel("Mega Number")
    ax3.set_ylabel("Weight")
    ax3.set_title("Mega Ball Recency-Weighted Frequency")
...
with tab3:
    ...
    if show_cooccur:
        ...
        ax4.set_title("White Pair Co-Occurrence (1..70)")
        fig4.colorbar(im4)
...
with tab4:
    ...
    with c1:
        ...
        df_fair = pd.DataFrame([{"whites": w, "mega": m} for w, m in fair])
    ...
    with c2:
        ...
        df_rec = pd.DataFrame([{"whites": w, "mega": m} for w, m in recency])
...
with tab5:
    ...
    def build_pdf_bytes():
        ...
        story.append(Paragraph("<b>Mega Millions Analysis Report</b>", styles["Title"]))
        story.append(
            Paragraph(
        ...
        story.append(Spacer(1, 6))
        story.append(Paragraph("<b>Top-5 Mega Balls (Recency)</b>", styles["Heading2"]))
        story.append(
            Paragraph(
        ...
        for w, m in fair:
            story.append(Paragraph(f"{w}  MB:{m:02d}", styles["Normal"]))
        story.append(Spacer(1, 6))
        story.append(
        ...
        if "img_mega" in locals() and img_mega:
            story.append(Paragraph("<b>Mega Ball (Recency)</b>", styles["Heading2"]))
    ...
    st.download_button(
        "📄 Download PDF report",
        data=pdf_bytes,
        file_name="MegaMillions_Report.pdf",
    ...
    def build_html_bytes():
        html = io.StringIO()
        html.write(
            "<html><head><meta charset='utf-8'><title>Mega Millions Report</title></head><body>"
        )
        html.write("<h1>Mega Millions Analysis Report</h1>")
        html.write(
        ...
        if "img_mega" in locals() and img_mega:
            html.write("<h2>Mega Ball (Recency)</h2>")
    ...
    st.download_button(
        ...
        data=html_bytes,
        file_name="MegaMillions_Report.html",
        mime="text/html",
    )

st.markdown("---")
st.caption(
    "Disclaimer: Mega Millions is random. These visuals highlight historical patterns and recency emphasis only, not predictive guarantees."
)
